...
> Logistic Regression is Regression used for Classification! 

## Regression again?

Classification is one of the most common Data Science business problems we encounter in real life. Because Odds are not bounded, but probability by definition is [0,1]. So math aside we define our problem as:  
...
$$
In the last equation we observe the famous *"Logit"* link function, therefore the **logistic regression model** has a logit that is linear in $X$. 
We can read the formulation as:

> *Increasing $X$ by one unit changes the log odds by \beta_1

But do not confuse *linearity in $X$ with linear regression, the change in $p(X)$ will depend on the current value $X$. 
Another important thing to remark is that $\beta_0$ is not of interest for statistical purposes so the significance or not is accessory. It is used only to adjust the average fitted probabilities to the proportion of the ones in the data.

```{r load Default}

Default <- fread(file = file.path('data', 'Default.csv'))
...
```{r log reg fit sample, out.width='100%'}

Default[, default := as.factor(default)]
Default[, student := as.factor(student)]
Default[, default_num := fifelse(default == 'No', 0, 1)]

model_balance <- glm(default_num ~ balance,
                     family = binomial(link = "logit"),
                     data = Default)

summary(model_balance)

df_model_balance = Default

df_model_balance[, predicted_default := predict(model_balance, type = "response")]


log_fit1 <-
ggplot(df_model_balance, 
       aes(x = balance,
           y = default_num)) + 
  geom_point(col = "#FCC76C", alpha = 0.5, size = 3) +
      geom_line(aes(y = predicted_default), col = "#FC776C", size = 1.5) 

log_fit2 <-
ggplot(df_model_balance, 
       aes(x = balance,
           y = default_num)) + 
  geom_jitter(col = "#FCC76C", alpha = 0.5, size = 3) +
      geom_line(aes(y = predicted_default), col = "#FC776C", size = 1.5) 

grid.arrange(log_fit1, log_fit2, ncol = 2)

```

## Multiple Logistic Regression

Now we will perform the same regression with all the variables (quantitative and qualitative) present in the dataset. 

```{r}
model_multi <- glm(default_num ~ balance + income + student,
                     family = binomial(link = "logit"),
                     data = Default)

df_model_multi = df_model_balance

df_model_multi[, predicted_default_m := predict(model_multi, type = "response")]
...
From the table above there are two things kind of eye catching:  
- `income` is non significative. It may be very correlated to `balance`.  
- `student` = **Yes** is negative.  

How is it possible for student status to be associated with an *decrease* in probability of default? The understanding of coefficients is conditioned, therefore, we can say that **for a fixed value of `balance` and `income` a student is less likely to default than a non student**. This is the so-called *Confounding phenomenon*. 

With the following graphs we will see whether the student characteristic has a positive or negative influence on defaulting:  

```{r}

df_model_multi[, default_rate := sum(default_num) / .N, by = c('student')]
...
Why we need a different method rather than logistic regression?  
- When there is substantial separation between the two classes parameter estimates tend to be surprisingly unstable.  
- If $X$ distributions are approximately Normal in each of the classes and **sample size is small**.  
- This methods are: *Linear Discriminant, Quadratic Discriminant, Naive Bayes*.  
